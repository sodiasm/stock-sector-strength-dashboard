...
def main():
    ...
    st.markdown(
        ...
        h1,h2,h3,h4,p,div,span,button,label { font-family:'DM Sans',sans-serif; }
    ...
    st.markdown(
        '<div class="warn-bar">This tool is for education and research. It places no real orders and is not investment advice.</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="page-header"><h2>Market Overview</h2><p>'
        "Read the market first: identify Risk-On, Risk-Off and sector rotation."
        "</p></div>", unsafe_allow_html=True)

    with st.sidebar:
        st.markdown("### Settings")
        custom = st.text_input("Ticker list (comma-separated)", "", placeholder="AAPL, MSFT, NVDA", key="sidebar_tickers")
        clean = sanitize_tickers(custom)
        tickers = clean or list(DEFAULT_TICKERS)
        if custom.strip() and not clean:
            st.warning("Invalid ticker input; reverted to the default list.")
        st.divider()
        st.caption(f"Research pool: {len(tickers)} stocks")
